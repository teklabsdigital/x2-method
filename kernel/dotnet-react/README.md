...
69 claims at catalog pass 2026-07-26: 21 `proven`, 6 `patterned`, 2 `latent`, 40 `owed`.

12 of those rows state a status per obligation; the row's own status is the weakest of them, so the tally above reads a split row at its weakest half and never at its strongest.

| Claim | Edition mechanism | Status |
|-------|-------------------|--------|
| TEN-1 tenant comes from the credential only | `TenantScopeMiddleware` reads `tenant_id` from the JWT only; `EndpointSpineTests` bans tenant route and query parameter names by whole-string case-insensitive equality, and bans DECLARED tenant/PII headers via the framework's own `IFromHeaderMetadata` with vendor-prefix and separator normalization; `HostSecurityTests` proves at run time that a forged tenant header in five spellings does not move a request into another tenant; `ContractShapeTests` bans `TenantId` on requests | owed (the weakest of 6 obligations); **the tenant is resolved solely from the validated credential** `proven`: `TenantScopeMiddleware` reads `tenant_id` from the token and from nothing else, which is the half of this claim the second edition cannot reach at all because it can mint no credential; **the route surface is enumerated completely** `proven`: the pattern loop reads parameter names without consulting a type, so no route parameter is invisible to it; what the predicate then does with the name is the comparison obligation below; **the query surface is enumerated completely** `owed`: parameter enumeration filters on `BindsFromUrl`, a hand-written list of types, so a value object implementing the framework's parse interface, or a `TimeSpan`, binds from the query string and yields nothing to the scan (E-2) (trigger: the next edition build pass); **the header surface has a mechanism of its own** `proven`: a header surface cannot be enumerated, so this is the non-scan mechanism ruling 5 requires, and it is covered from both sides as of 2026-07-26: a DECLARED header binding is a scan violation through the framework's own `IFromHeaderMetadata`, with vendor-prefix and separator normalization, and a header arriving UNDECLARED is proven at run time not to move a request into another tenant, in five spellings (A-2); **the request-contract surface is enumerated completely** `owed`: the contract half rests on `ContractShapeTests`, which is flat and enumerates only `*Request`-suffixed types, so a tenant id nested inside a posted object, or carried by a type whose name it does not recognize, is not reached (E-6) (trigger: MOD-2's delta pass); **the comparison resolves compounds, concatenations and decompositions of a listed word** `owed`: matching is whole-string case-insensitive equality against a registry; the flow-back pass widened that registry with `workspace` and `account` and their compounds, which is a longer list and not a comparison (E-9) (trigger: the next edition build pass); TEN-1 names four surfaces and this edition now reaches all four, unevenly. ROUTE is total, because the pattern loop reads parameter names without consulting a type. HEADER is covered from both sides as of 2026-07-26 (A-2): a declared binding is a scan violation, and a header arriving undeclared is proven not to influence tenant resolution, which is the half no scan can reach. QUERY remains type-dependent: parameter enumeration filters on `BindsFromUrl`, a hand-written enumeration of types, so an `IParsable` value object or a `TimeSpan` binds from the query string and yields nothing to the scan (E-2, not repaired: one predicate cannot serve SEC-2 and SEC-3/TEN-1 at once). Matching is equality against a registry, so a novel spelling walks past (E-9). Findings: A-2 (repaired), E-2 (open), E-9. **Lowered from `proven` to `owed` by the adjudication pass of 2026-07-26 (rulings 5 and 6), through ruling 3's per-obligation structure.** Ruling 5 predicted the header surface would be the falsifying half; it is not, because the flow-back pass built the header mechanism after that section of the digest was drafted, and it reads `proven` above. What falsifies the row is the query enumeration (E-2) and the comparison (E-9), both measured before the ruling and both left open by name. |
...
| SEC-2 anti mass-assignment | `ContractShapeTests` forbidden-field registry over the Contracts assembly, filtered to types whose name ends in `Request` (FLAT); `EndpointSpineTests` walks every body-bound DTO RECURSIVELY, through public properties, constructor parameters, arrays and generic arguments, with cycle protection and no depth cap, against the same registry; App/Api declare no request/response types | owed (the weakest of 3 obligations); **the predicate rejects a registry member at any depth of an enumerated type** `proven`: `EndpointSpineTests` walks every body-bound DTO recursively through public properties, constructor parameters, arrays and generic arguments, with cycle protection and no depth cap, proven both ways against a nested `AuthorDto` carrying `CreatedBy` (E-6); **the enumeration reaches every body-bound type** `owed`: `IsBodyDto` excludes types with a null namespace, so a public DTO declared in the entry-point file is outside the host scan, and `BindsFromUrl` over-excludes the body set (E-2, E-6). The second net, `ContractShapeTests`, is flat and enumerates only `*Request`-suffixed types, so it reaches one level of the types whose name it recognizes (trigger: MOD-2's delta pass, which owns the placement rule that flat net rests on); **the comparison resolves compounds, concatenations and decompositions of a listed word** `owed`: matching is a lowercased hash-set lookup, which is equality: a registry entry `status` does not cover `noteStatus`, and widening the list with more spellings is enumeration standing in for a comparison (E-9) (trigger: the next edition build pass, where E-2's split of the one predicate now serving SEC-2 and SEC-3/TEN-1 is the same work); The two guards are no longer equivalent and the difference is the point. The HOST scan recurses as of 2026-07-26 (E-6), so a server-controlled field at any depth of any body-bound type on a real route is caught, proven both ways against a nested `AuthorDto` carrying `CreatedBy`. `ContractShapeTests` is STILL FLAT and still enumerates only `*Request` types, so as a second net it reaches one level; repairing it belongs to MOD-2, which has not had its delta pass. A reader taking `proven` to mean both guards reach every depth would be wrong. `IsBodyDto` also excludes types with a null namespace, so a public DTO declared in `Program.cs` is skipped by the host scan entirely (E-6), and `BindsFromUrl` over-excludes the body set (E-2). Findings: E-2 (open), E-6 (host half repaired, contracts half blocked on MOD-2). **Lowered from `proven` to `owed` by the adjudication pass of 2026-07-26 (rulings 2 and 6), through ruling 3's per-obligation structure rather than by collapsing to one word.** No mechanism named above changed and nothing new was discovered: the guards are exactly as measured, and what changed is that the claim now states a completeness obligation and a comparison obligation, and this edition meets neither. The recursive host scan is a real half and reads `proven` above. |
| SEC-3 no PII in URLs | `EndpointSpineTests` route + query scan ([AsParameters] wrappers, collections, DateTime/TimeOnly) | owed (the weakest of 3 obligations); **the predicate rejects a registry match on a parameter it can see** `proven`: `EndpointSpineTests` scans route and query parameters, covering `[AsParameters]` wrappers, collections and date parameter types that a naive scan misses; **the enumeration reaches every route and query parameter** `owed`: query-parameter enumeration filters on `BindsFromUrl`, a hand-written list of types, so a value object implementing the framework's own parse interface binds from the query string and yields nothing to the scan (E-2) (trigger: the next edition build pass); **the comparison resolves compounds, concatenations, decompositions and plurals of a listed word** `owed`: matching is case-insensitive equality, so `emailAddress` walks past an entry `email` and five further spellings of the same value walk past with it (E-9) (trigger: the next edition build pass, with E-2's predicate split); **Lowered from `proven` to `owed` by the adjudication pass of 2026-07-26 (ruling 6), through ruling 3's per-obligation structure.** The scan is unchanged; the claim now requires the comparison to resolve morphology, and case-insensitive equality does not. The sibling edition's SEC-3 survives because it built the three morphological classes explicitly, which is the asymmetry the evidence says exists. |
...
| TIME-1 UTC, offset-aware, and nothing else | `TimeTypeTests` over Contracts, App, Persistence (recurses Nullable, arrays, generics) | owed (the weakest of 3 obligations); **the predicate rejects a naive instant in the layers it reaches** `proven`: `TimeTypeTests` recurses Nullable, arrays and generics over the layers it is given, and rejects naive `DateTime` in all of them; **the enumeration reaches every layer that declares a time value** `owed`: the layer set is a hand-written list of three and the API layer is not in it, measured with a control: a naive `DateTime` on a public type there leaves the whole suite green, while the identical type one layer over turns it red (E-15). The repair is one line and was held deliberately for this ruling rather than for difficulty (trigger: the next edition build pass); **durations come from the monotonic source through one clock seam** `owed`: the wall-clock-delta lint the 2026-07-24 extension adds has no realization here, and nothing confines a now-reading to a seam (trigger: the next edition build pass); DB UTC column defaults not used: the service sets the value **Lowered from `proven` to `owed` by the adjudication pass of 2026-07-26 (rulings 2 and 3).** E-15 measured the omitted layer with a control and the flow-back pass deliberately did not repair it, on the grounds that lowering the row belonged to the owner; this is where that landed. The monotonic-durations half has never had a realization here and now says so in its own status rather than in prose. |
