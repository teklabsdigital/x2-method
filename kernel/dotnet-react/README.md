...
| DEP-1 dependency quarantine | CPM exact pins + lockfiles + locked-mode restore (`--locked-mode` / `npm ci`) + `VERSIONS.md` (30-day window header, container-images section, kernel-provenance section) + docs-lint ledger, image, and kernel-provenance checks; the SQL Server image is one tag@digest value across runbook, CI, and the Testcontainers fixture | proven; publish-date CI check owed (dates recorded by hand); kernel-provenance check latent until the next instantiation fills a real pin; the CI toolchain itself is unpinned and carries no ledger rows (E-1, not repaired). The shared client tier was re-planned against the 30-day window on 2026-07-26 (E-3): 14 advisories to 1 root, 0 critical, verified by `npm ci` and `npm run verify` in both editions. The one survivor is `postcss` GHSA-r28c-9q8g-f849, which has NO release satisfying both the window and the advisory; it is pinned to the newest in-window release through an overrides entry and named in VERSIONS.md rather than resolved silently (E-18). DEP-1 still states no resolution order for that case. |
