...
| SEC-4 JWT hardening and revocation | pinned HS256, `RequireSignedTokens`, `SessionVersionMiddleware`; `HostSecurityTests` rejects a wrong-algorithm token and a missing-sv token | proven (the weakest of 2 obligations); **the signing algorithm is pinned and tokens must be signed** `proven`: pinned HS256 plus `RequireSignedTokens`; `HostSecurityTests` rejects a wrong-algorithm token rather than asserting the option is set; **revocation is immediate through a server-side session version** `proven`: `SessionVersionMiddleware` plus the missing-`sv` rejection test. The v1 version store is process-local, so an EF-backed store is this mechanism's named upgrade and not a second obligation: the claim asks that revocation be immediate, which it is; EF-backed session store owed (trigger: first identity slice) |
...
| SEC-6 log safety | `redact.ts` scrubs Authorization and JWT-shaped strings; `redact.test.ts` | patterned (the weakest of 2 obligations); **secret-shaped values are scrubbed by one redactor** `proven`: `redact.ts` scrubs Authorization headers and JWT-shaped strings; `redact.test.ts` proves the scrub on both; **each logging surface asserts its own redaction** `patterned`: the v1 surfaces carry their tests; every new logging surface owes one, and the hub is the next (trigger: realtime); v1 surfaces tested; hub log-safety owed (trigger: realtime) |
...
| UI-4 fidelity ledger | `NotesScreen.fidelity.test.tsx` exhaustiveness + de-fabrication; ledgers live in `design/ledger/` (INV-01) | patterned (the weakest of 2 obligations); **every ledger atom renders** `patterned`: `NotesScreen.fidelity.test.tsx` asserts exhaustiveness against the ledger; each new screen owes its own suite; **nothing renders beyond the ledger** `patterned`: the same suite asserts de-fabrication, which is the direction a generated UI fails in without anyone noticing; each screen owes its suite; per-atom resolved-style assertions not included; the prototype-to-ledger exporter owed |
...
| DEP-1 dependency quarantine | CPM exact pins + lockfiles + locked-mode restore (`--locked-mode` / `npm ci`) + `VERSIONS.md` (30-day window header, container-images section, kernel-provenance section) + docs-lint ledger, image, and kernel-provenance checks; the SQL Server image is one tag@digest value across runbook, CI, and the Testcontainers fixture | proven (the weakest of 2 obligations); **every direct dependency is pinned exactly, dated, and restored in locked mode** `proven`: CPM exact pins plus committed lockfiles plus `--locked-mode` and `npm ci`, with the docs-lint ledger check keyed on name AND version so a row for one version vouches for no other; **every container image is pinned tag-plus-digest and ledgered** `proven`: the SQL Server image is one tag@digest value across runbook, CI and the Testcontainers fixture; docs-lint fails a reference that floats or that has no ledger row (INV-05); publish-date CI check owed (dates recorded by hand); kernel-provenance check latent until the next instantiation fills a real pin; the CI toolchain itself is unpinned and carries no ledger rows (E-1, not repaired). The shared client tier was re-planned against the 30-day window on 2026-07-26 (E-3): 14 advisories to 1 root, 0 critical, verified by `npm ci` and `npm run verify` in both editions. `postcss` GHSA-r28c-9q8g-f849 had no release satisfying both the window and the advisory, and DEP-1 gained the resolution order at adjudication ruling 9a on 2026-07-26: the advisory outranks the window above the 7-day floor, by explicit decision, with a ledger row for the pin whether it is direct or transitive. The override moved to 8.5.18 in the shared tier and `npm audit` over that tier now reports 0 vulnerabilities, down from 1 high. The pin is the first row of VERSIONS.md's advisory-rule section and leaves it on 2026-08-11 (E-18). |
...
| TEST-3 CI loop | `ci.yml`: server, client, docs-lint, secret-scan, e2e-wire | owed (the weakest of 2 obligations); **every mechanism the catalog names executes automatically in CI on every push and pull request** `patterned`: `ci.yml` declares server, client, docs-lint, secret-scan and e2e-wire. The workflow has never executed on a forge, and one of the five jobs is red on day one in a seeded project for a missing allowlist the file set does not carry (E-25); **the loop gates merges, armed at instantiation, with the arming step carried and verified** `owed`: no arming artifact exists, and part C`s verification item names no command, needs a second identity, and leaves nothing in the seeded repo recording that it passed (E-24). Trigger: the same ruleset writer and readback HUM-1 names; Lowered from `proven` to `owed` (the weakest of 2 obligations) on 2026-07-27 by the acceptance test. The loop exists and its five jobs are real; what has never existed is the arming step the whole conditional rests on. TEST-3`s own weakening note predicts this defect verbatim, that the acceptance test must verify instantiation actually arms the gate rather than that the workflow file exists. It ran, and instantiation ships nothing to verify (E-24). Two further measurements from the same run: the `secret-scan` job cannot go green in a seeded project because the manifest`s file set omits `secret-scan.allow.json`, and part C never runs that job, so a red required gate is invisible to the set that exists to catch it (E-25). The roll-up is the weakest obligation, so a built loop that gates nothing reads `owed`, which is the conservative direction the ruling names. Findings: E-24, E-25. |
...
| HUM-1 irreversible surfaces get a human turn, unconditionally | `.github/CODEOWNERS` covers migrations, contracts and `docs/contracts/`, reconciled against `edition.json``s `irreversibleSurfaces` so renaming one and forgetting the other is loud; docs-lint fails if any of the three loses its owned entry, and fails outside the kernel repo if an entry names only the `@OWNER` placeholder; branch protection requires code-owner review | owed (the weakest of 2 obligations); **every irreversible surface is enumerated and declared to require a named human review** `proven`: the three surfaces are declared in `edition.json` and reconciled against `.github/CODEOWNERS`, and an entry naming only the shipped placeholder is now refused outside the kernel repo. Red-green proven in a seeded tree: placeholder gives three failures, one per surface, each naming the path and the claim; a real handle gives ok; the kernel itself stays green because its placeholders are legal there; and the pre-repair predicate with the placeholder still in place goes green again, which reproduces the hole (E-23); **the merge gate requires that review, armed at instantiation** `owed`: measured absent 2026-07-27: the repository contains no forge API call, no ruleset, no token and no readback, and every hit for branch protection is prose. The acceptance test can verify only what instantiation ships, and instantiation ships an imperative sentence a human executes by hand in a web interface (E-24). Trigger: a ruleset writer plus an armed-state readback, which is also a decision about which forge the kernel presumes; Lowered from `proven` to `owed` (the weakest of 2 obligations) on 2026-07-27 by the first execution of the instantiation acceptance test, which measured both halves rather than reading them. The declaration half was a FALSE GREEN and is now genuinely proven: docs-lint asked only whether an owner-shaped token covered each surface, and `@OWNER` is owner-shaped, so a seeded project with all four placeholders unreplaced passed with every irreversible surface owned by nobody (E-23). Repaired by separating covered from owned, reusing DEP-1`s kernel-context detection in the same file so the placeholder stays legal in the kernel and is refused downstream; red-green proven in a seeded tree with the control that reproduces the hole. The arming half has no artifact at all: instantiation ships one sentence of prose, no script, no ruleset, no readback, and part C`s verification item names no command and leaves no trace (E-24). The roll-up is the weakest obligation, so the row reads `owed` even though its declaration half is now genuinely proven; `proven` beside `mechanism built, not yet armed` was the mismatch all along. Findings: E-23 (repaired), E-24 (recorded, not repaired). |
