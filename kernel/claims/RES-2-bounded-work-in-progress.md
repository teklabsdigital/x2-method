...
**Weakening notes.** The right bound is contextual and the claim does not pretend otherwise: bound values are D-000 rulings held as CFG-1 operational settings, and load testing is how they are trued. The claim rules the shape (a bound must exist and a policy must be named), not the number. Distributed queues are RES-5's territory; this claim ends at the process boundary.
