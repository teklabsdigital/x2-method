...
**Enforcement.**
- Mechanism class: banned-API lint scoped to the server's own code, in whatever unit that stack compiles or ships it as (sync file and network APIs, sync-over-async idioms, read-to-end idioms), with the startup allowlist named in the lint configuration; a configuration test asserts the platform's sync-IO runtime throw is intact where the platform has one.
