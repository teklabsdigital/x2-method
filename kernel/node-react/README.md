...
69 claims at catalog pass 2026-07-26: 3 `proven`, 1 `patterned`, 0 `latent`, 65 `owed`.

8 of those rows state a status per obligation; the row's own status is the weakest of them, so the tally above reads a split row at its weakest half and never at its strongest.
